#!/usr/bin/env python3
"""
ShelfQuest CSV importer v0.1.4.
...
- Duplicate temporary ISBN barcode rows are counted into owned_qty, because they represent physical duplicates during the ISBN-only phase.
- Duplicate explicit custom Barcode rows are still skipped/reported because a custom barcode must identify one physical copy.
"""
import argparse
import csv
import json
...
def import_csv(csv_path, dry_run=False, lookup_missing=False):
    ...
    # Pre-count ISBN/title rows so owned_qty can reflect row-per-copy spreadsheets.
    # This matters during the ISBN-only phase: multiple physical copies scan with the
    # same ISBN, so only one scannable copy can be created, but owned_qty should still
    # record the actual number owned.
    barcodes_by_isbn = defaultdict(set)
    max_qty_by_isbn = defaultdict(lambda: 1)
    row_count_by_isbn = defaultdict(int)
    prepared_rows = []

    for idx, row in enumerate(rows, start=2):
        isbn = clean_isbn(get_field(row, "ISBN"))
        raw_barcode = clean_text(get_field(row, "Barcode"))
        has_explicit_barcode = bool(raw_barcode)
        barcode = clean_barcode(raw_barcode, fallback_isbn=isbn)
        qty = clean_int(get_field(row, "Qty"), 1)
        if isbn:
            row_count_by_isbn[isbn] += 1
            if barcode:
                barcodes_by_isbn[isbn].add(barcode)
            max_qty_by_isbn[isbn] = max(max_qty_by_isbn[isbn], qty)
        prepared_rows.append((idx, row, isbn, barcode, qty, has_explicit_barcode))
    ...
    try:
        for idx, row, isbn, barcode, qty, has_explicit_barcode in prepared_rows:
            ...
            if barcode in seen_barcodes:
                # If the duplicate scan code is just the ISBN fallback, this is a normal
                # ISBN-only duplicate-copy situation. Do not create another copy with
                # the same barcode; count it via books.owned_qty instead.
                if isbn and (not has_explicit_barcode or barcode == isbn):
                    duplicate_rows_counted_as_qty.append((idx, barcode, title or "<blank title>"))
                    continue
            ...
            if isbn:
                owned_qty = max(
                    qty,
                    max_qty_by_isbn[isbn],
                    row_count_by_isbn[isbn],
                    len(barcodes_by_isbn[isbn]) or 1,
                )
            else:
                owned_qty = qty
    ...
    print(f"Duplicate ISBN rows counted as Qty:{len(duplicate_rows_counted_as_qty)}")
    print(f"Duplicate explicit barcode rows skipped:{len(duplicate_barcode_rows)}")
    ...
    print("\nBarcode handling: Barcode is the scannable copy code. If blank, ISBN is used. Duplicate ISBN-only rows update Qty but do not create duplicate scannable copies.")

    if duplicate_rows_counted_as_qty:
        print("\nDuplicate ISBN-only rows counted as quantity, not imported as separate scannable copies:")
        for line, barcode, title in duplicate_rows_counted_as_qty[:30]:
            print(f"  line {line}: {barcode} - {title}")
        if len(duplicate_rows_counted_as_qty) > 30:
            print(f"  ... plus {len(duplicate_rows_counted_as_qty) - 30} more")

    if duplicate_barcode_rows:
        print("\nDuplicate explicit Barcode rows skipped:")
